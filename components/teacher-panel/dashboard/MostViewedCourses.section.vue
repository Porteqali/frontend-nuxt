...
<template>
    <section class="flex flex-col gap-6 p-4 md:p-6 w-screen max-w-full">
        <header class="flex flex-wrap items-center justify-between gap-4">
            <h3 class="relative text-2xl"><b class="relative z-10">پر بازدید ترین دوره ها</b></h3>
            <div class="flex">
            ...
            <ul class="swiper-wrapper flex items-center w-screen pb-4">
                <li class="swiper-slide course_card flex flex-col gap-2 shadow-md p-1 ml-4 rounded-2xl bg-gray-50" v-for="(item, i) in courses" :key="i">
                    ...
                    <div class="flex items-start gap-2 p-1">
                        ...
                        <span class="text-xl text-blue-900">{{ new Intl.NumberFormat("fa").format(item.viewCount) }}</span>
                    </div>
                </li>
            </ul>
...
export default {
    ...
    methods: {
        async getCourses(data = {}) {
            ...
            let url = `/api/teacher-panel/dashboard/most-viewed-courses?period=${this.period.value}`;
            let headers = {};
            if (process.server) {
