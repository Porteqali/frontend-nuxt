...
<template>
    <Dialog :open="open" title="ویرایش و تایید شماره موبایل" @update:open="updateOpenState">
        <div class="flex flex-col gap-4 w-screen max-w-xs md:max-w-md">
            <div class="flex flex-wrap md:flex-nowrap items-end gap-4 w-full">
                <div class="flex flex-col gap-2 w-full flex-grow">
                    ...
                    <input type="text" v-model="mobile" name="mobile" v-mask="'09#########'" :masked="true" dir="auto" class="p-3 w-full bg-warmgray-100 rounded-xl focus:shadow-xl" />
                </div>
...
export default {
    ...
    methods: {
        ...
        async sendVerificationCode() {
            ...
            await axios
                .post(`/api/users/send-verification-code`, { username: this.mobile, type: "mobile" })
                .then((response) => {
        ...
        async verify() {
            ...
            await axios
                .post(`/api/users/verify`, { username: this.mobile, code: this.code, type: "mobile" })
                .then(async (response) => {
                    this.timeLeft = 0;
                    ...
                    this.$store.commit("user/setInfo", { ...this.user.info, mobile: this.mobile });
                    this.updateOpenState(false);
                    this.$emit("update:mobile", this.mobile);
                })
                .catch((e) => {
                    if (typeof e.response !== "undefined" && e.response.data) {
                        if (typeof e.response.data.message === "object") {
