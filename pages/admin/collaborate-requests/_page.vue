    ...
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap justify-between items-center gap-4">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-2xl"><b>درخواست های همکاری</b></h1>
            </div>
            <!--  -->
        </div>
        ...
        <Table
            ...
            :total="total"
            :pageTotal="pageTotal"
            :pageUrl="`/admin/collaborate-requests/:page?search=${search}`"
        ...
        >
            <template v-slot:tbody="{ record, index }">
                ...
                <td>
                    <span class="title">عنوان پیشنهادی</span>
                    <span>{{ record.suggestiveRole }}</span>
                </td>
                <td>
                    <p>{{ record.description.length > 60 ? record.description.substr(0, 60) + "..." : record.description }}</p>
                </td>
                <td>
                    <span class="p-1 px-2 text-xs rounded-md bg-emerald-100 text-emerald-700" v-if="record.status == 'new'">جدید</span>
                    <span class="p-1 px-2 text-xs rounded-md bg-lightblue-100 text-lightblue-700" v-if="record.status == 'viewed'">مشاهده شده</span>
                </td>
                ...
                <td>
                    <div class="flex items-center gap-1">
                        <router-link
                            ...
                            title="مشاهده"
                            :to="`/admin/collaborate-requests/details/${record._id}`"
                            v-if="checkPermissions(['admin.collaborate-requests.view'], userPermissions)"
                        >
                        ...
                        <button
                            ...
                            v-if="checkPermissions(['admin.collaborate-requests.delete'], userPermissions)"
                        >
        ...
        <DeleteDialog
            ...
            url="/api/admin/collaborate-requests"
        />
    </main>
</template>
...
export default {
    layout: "admin",
    head() {
        return { title: "درخواست های همکاری - گروه آموزشی پرتقال" };
    },
    ...
    methods: {
        async getTableData(data = {}) {
            ...
            let url = `/api/admin/collaborate-requests`;
            let headers = {};
            if (process.server) {
