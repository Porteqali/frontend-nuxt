...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <nuxt-link to="/admin/collaborate-requests">درخواست های همکاری</nuxt-link>
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>مشاهده درخواست</b></h1>
        </div>
        ...
        <section class="flex flex-col gap-4 bg-white rounded-2xl shadow-lg mx-auto w-full max-w-screen-lg flex-grow p-4">
            <form class="flex flex-col gap-4 p-1 flex-grow max-h-full overflow-auto">
                ...
                <hr class="w-full" />
                <label class="">
                    <small>عنوان پیشنهادی برای تدریس:</small>
                    {{ suggestiveRole }}
                </label>
                <p class="bg-warmgray-50 rounded-xl p-2 w-full h-full">{{ description }}</p>
            </form>
            <hr class="w-full" />
...
export default {
    layout: "admin",
    head() {
        return { title: "مشاهده درخواست - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    methods: {
        async loadRequest(data = {}, route) {
            let url = `/api/admin/collaborate-requests/${route.params.id}`;
            let headers = {};
            if (process.server) {
            ...
            await axios
                ...
                .then((response) => {
                    ...
                    this.suggestiveRole = response.data.suggestiveRole;
