...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>ویرایش تخصص</b></h1>
        </div>
        ...
        <section class="flex flex-col gap-4 bg-white rounded-2xl shadow-lg mx-auto w-full max-w-screen-lg flex-grow p-4">
            <form class="flex flex-col gap-4 p-1 flex-grow max-h-full overflow-auto">
                ...
                <hr class="w-full" />
                <nuxt-link :to="`/admin/majors/bundles/${$route.params.id}`" class="w-max bg-lightblue-100 p-2 py-1 rounded-lg text-lightblue-900">
                    <small class="opacity-75 hover:underline">مشاهده و ویرایش باندل های این تخصص</small>
                </nuxt-link>
            </form>
            <hr class="w-full" />
...
export default {
    layout: "admin",
    head() {
        return { title: "ویرایش تخصص - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    async fetch() {
        ...
        await this.loadMajor({ headers }, route);
    },
    mounted() {
    ...
    methods: {
        async loadMajor(data = {}, route) {
            let url = `/api/admin/majors/${route.params.id}`;
            let headers = {};
            if (process.server) {
        ...
        async save() {
            ...
            let url = encodeURI(`/api/admin/majors/${this.$route.params.id}`);
            await axios
                .put(url, formData, { onUploadProgress: (e) => (this.uploadingFilesPercentage = parseInt(Math.round((e.loaded / e.total) * 100))) })
                .then((response) => {
                    this.$store.dispatch("toast/makeToast", { type: "success", title: "", message: "تخصص با موفقیت ویرایش شد" });
                })
                .catch((e) => {
                    if (typeof e.response !== "undefined" && e.response.data && typeof e.response.data.message === "object") {
                        this.$store.dispatch("toast/makeToast", { type: "error", title: "خطا", message: e.response.data.message[0].errors[0] });
