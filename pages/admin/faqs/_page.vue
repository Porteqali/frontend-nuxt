    ...
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap justify-between items-center gap-4">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-2xl"><b>سوالات متداول</b></h1>
            </div>
            <nuxt-link
                to="/admin/faqs/create"
                class="orange_gradient_v rounded-xl p-2 px-4 w-max hover:shadow-md"
                v-if="checkPermissions(['admin.faqs.add'], userPermissions)"
            >
                سوال جدید
            </nuxt-link>
        </div>
        ...
        <Table
            ...
            :total="total"
            :pageTotal="pageTotal"
            :pageUrl="`/admin/faqs/:page?search=${search}`"
            @update:table="getTableData()"
        >
            <template v-slot:tbody="{ record, index }">
                <td>
                    <p>{{ record.question.length > 60 ? record.question.substr(0, 60) + "..." : record.question }}</p>
                </td>
                <td>
                    <p>{{ record.answer.length > 60 ? record.answer.substr(0, 60) + "..." : record.answer }}</p>
                </td>
                <td>
                    <span class="p-1 px-2 text-xs rounded-md bg-emerald-100 text-emerald-700" v-if="record.status == 'published'">منتشر شده</span>
                    <span class="p-1 px-2 text-xs rounded-md bg-rose-100 text-rose-700" v-if="record.status == 'pending'">منتظر انتشار</span>
                </td>
                ...
                <td>
                    <div class="flex items-center gap-1">
                        <router-link
                            ...
                            title="Edit"
                            :to="`/admin/faqs/edit/${record._id}`"
                            v-if="checkPermissions(['admin.faqs.edit'], userPermissions)"
                        >
                        ...
                        <button
                            ...
                            v-if="checkPermissions(['admin.faqs.delete'], userPermissions)"
                        >
...
export default {
    layout: "admin",
    head() {
        return { title: "سوالات متداول - گروه آموزشی پرتقال" };
    },
