...
export default {
    ...
    async fetch() {
        ...
        await this.loadFaq({ headers }, route);
    },
    mounted() {
    ...
    methods: {
        async loadFaq(data = {}, route) {
            let url = `/api/admin/faqs/${route.params.id}`;
            let headers = {};
            if (process.server) {
            ...
            await axios
                ...
                .then((response) => {
                    ...
                    this.group = this.groupOptions[response.data.group] || { name: "", value: "" };
                    this.status = this.statusOptions[response.data.status] || { name: "", value: "" };
                })
                .catch((e) => {
        ...
        async save() {
            ...
            let url = encodeURI(`/api/admin/faqs/${this.$route.params.id}`);
            await axios
                .put(url, {
