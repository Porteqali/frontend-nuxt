        ...
        <section class="flex flex-col gap-4 bg-white rounded-2xl shadow-lg mx-auto w-full max-w-screen-lg flex-grow p-4">
            <form class="flex flex-col gap-4 p-1 flex-grow max-h-full overflow-y-auto" @submit="$event.preventDefault()">
                ...
                <div class="relative flex flex-col w-full">
                    ...
                    <ul class="absolute top-16 flex flex-col gap-2 p-2 pt-6 rounded-xl w-full bg-white shadow-lg z-10" v-if="!!courseSearchQuery">
                        <li
...
export default {
    ...
    methods: {
        ...
        async save() {
            ...
            if (this.applyDateFilter) formData.append("startDate", this.startDate);
            if (this.applyDateFilter) formData.append("endDate", this.endDate);
            if (this.applyCourseFilter) formData.append("selectedCourses", this.selectedCourses);
            formData.append("mobileField", this.mobileField);
            formData.append("emailField", this.emailField);

            let url = encodeURI(`/api/admin/users/export`);
            await axios
                .get(url)
                .then((response) => {
