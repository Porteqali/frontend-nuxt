...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>ویرایش ادمین</b></h1>
        </div>
...
export default {
    layout: "admin",
    head() {
        return { title: "ویرایش ادمین - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    async fetch() {
        ...
        await this.getPermissionGroups({ headers });
        await this.loadAdmin({ headers }, route);
    },
    computed: {
        userPermissions() {
    ...
    methods: {
        ...
        async loadAdmin(data = {}, route) {
            let url = `/api/admin/list/${route.params.id}`;
            let headers = {};
            if (process.server) {
                url = `${process.env.BASE_URL}${url}`;
                headers = data.headers ? data.headers : {};
            ...
            await axios
                ...
                .then((response) => {
                    ...
                    this.permissionGroup = this.permissionGroups[response.data.permissionGroup];
        ...
        async save() {
            ...
            let url = encodeURI(`/api/admin/list/${this.$route.params.id}`);
            await axios
                .put(url, formData)
                .then((response) => {
                    this.$store.dispatch("toast/makeToast", { type: "success", title: "", message: "ادمین با موفقیت ویرایش شد" });
                })
                .catch((e) => {
                    if (typeof e.response !== "undefined" && e.response.data && typeof e.response.data.message === "object") {
                        this.$store.dispatch("toast/makeToast", { type: "error", title: "خطا", message: e.response.data.message[0].errors[0] });
