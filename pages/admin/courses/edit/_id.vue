...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>ویرایش دوره</b></h1>
        </div>
        ...
        <section class="flex flex-col gap-4 bg-white rounded-2xl shadow-lg mx-auto w-full max-w-screen-lg flex-grow p-4">
            <form class="flex flex-col gap-4 p-1 flex-grow max-h-full overflow-auto">
                ...
                <hr class="w-full" />
                <nuxt-link :to="`/admin/courses/topics/${$route.params.id}`" class="w-max bg-lightblue-100 p-2 py-1 rounded-lg text-lightblue-900">
                    <small class="opacity-75 hover:underline">مشاهده و ویرایش سرفصل ها دوره</small>
                </nuxt-link>
            </form>
            <hr class="w-full" />
...
export default {
    layout: "admin",
    head() {
        return { title: "ویرایش دوره - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    async fetch() {
        ...
        await this.loadCourse({ headers }, route);
    },
    computed: {
        userPermissions() {
    ...
    methods: {
        ...
        async loadCourse(data = {}, route) {
            let url = `/api/admin/courses/${route.params.id}`;
            let headers = {};
            if (process.server) {
                url = `${process.env.BASE_URL}${url}`;
                headers = data.headers ? data.headers : {};
            ...
            await axios
                ...
                .then((response) => {
                    ...
                    this.selectedTeacher = {
                        ...
                        fullname: `${response.data.teacher.name} ${response.data.teacher.family}`,
                    };
                    ...
                    this.commission = this.commissionOptions[response.data.commission] || { name: "", value: "" };
                    this.tags = response.data.tags || [];
                    this.showInNew = response.data.showInNew;
                    for (let i = 0; i < response.data.exerciseFiles.length; i++) this.exerciseFiles.push({ ...response.data.exerciseFiles[i], fileRaw: null });
                })
                .catch((e) => {
        ...
        async save() {
            ...
            const RemainedExerciseFilesIds = [];
            for (let i = 0; i < this.exerciseFiles.length; i++) {
                ...
                if (this.exerciseFiles[i]._id) RemainedExerciseFilesIds.push(this.exerciseFiles[i]._id);
            }
            formData.append("exerciseFilesDetails", JSON.stringify(exerciseFilesDetails));
            formData.append("RemainedExerciseFilesIds", JSON.stringify(RemainedExerciseFilesIds));
