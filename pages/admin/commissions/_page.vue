    ...
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap justify-between items-center gap-4">
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-2xl"><b>کمیسیون ها</b></h1>
            </div>
            <nuxt-link
                to="/admin/commissions/create"
                class="orange_gradient_v rounded-xl p-2 px-4 w-max hover:shadow-md"
                v-if="checkPermissions(['admin.commissions.add'], userPermissions)"
            >
                کمیسیون جدید
            </nuxt-link>
        </div>
        ...
        <Table
            ...
            :total="total"
            :pageTotal="pageTotal"
            :pageUrl="`/admin/commissions/:page?search=${search}`"
        ...
        >
            <template v-slot:tbody="{ record, index }">
                ...
                <td>
                    <span v-if="record.type == 'percent'">درصد</span>
                    <span v-if="record.type == 'number'">مبلغ ثابت</span>
                </td>
                <td>
                    {{ new Intl.NumberFormat("fa").format(record.amount) }}
                    <small v-if="record.type == 'percent'">%</small>
                    <small v-if="record.type == 'number'">تومان</small>
                </td>
                ...
                <td>
                    <div class="flex items-center gap-1">
                        <router-link
                            ...
                            title="Edit"
                            :to="`/admin/commissions/edit/${record._id}`"
                            v-if="checkPermissions(['admin.commissions.edit'], userPermissions)"
                        >
                        ...
                        <button
                            ...
                            v-if="checkPermissions(['admin.commissions.delete'], userPermissions)"
                        >
...
export default {
    layout: "admin",
    head() {
        return { title: "کمیسیون ها - گروه آموزشی پرتقال" };
    },
