...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <nuxt-link to="/admin/commissions">کمیسیون ها</nuxt-link>
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>ویرایش کمیسیون</b></h1>
        </div>
            ...
            <form class="flex flex-col gap-4 flex-grow max-h-full overflow-auto">
                ...
                <div class="flex flex-wrap md:flex-nowrap items-end gap-4 w-full">
                    <div class="flex flex-col gap-2 w-max flex-shrink-0">
                        <label for=""></label>
                        <div class="flex items-center gap-2 cursor-pointer select-none" @click="type = 'percent'">
                            <transition name="check" mode="out-in" appear>
                                <img src="/icons/admin/TickSquare.svg" width="24" v-if="type == 'percent'" />
                                <img src="/icons/admin/TickSquareBox.svg" width="24" v-else />
                            </transition>
                            <span class="text-sm opacity-75">درصد</span>
                        </div>
                        <div class="flex items-center gap-2 cursor-pointer select-none" @click="type = 'number'">
                            <transition name="check" mode="out-in" appear>
                                <img src="/icons/admin/TickSquare.svg" width="24" v-if="type == 'number'" />
                                <img src="/icons/admin/TickSquareBox.svg" width="24" v-else />
                            </transition>
                    ...
                    <div class="flex flex-col gap-2 w-full">
                        ...
                        <div class="flex items-center gap-2 p-3 w-full rounded-xl shadow-sm focus:shadow-md bg-coolgray-100">
                            ...
                            <small class="font-bold" v-if="type == 'percent'">%</small>
                            <small class="font-bold" v-if="type == 'number'">تومان</small>
                        </div>
                    </div>
                </div>
            </form>
...
export default {
    layout: "admin",
    head() {
        return { title: "ویرایش کمیسیون - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    async fetch() {
        ...
        await this.loadCommission({ headers }, route);
    },
    mounted() {
    ...
    methods: {
        async loadCommission(data = {}, route) {
            let url = `/api/admin/commissions/${route.params.id}`;
            let headers = {};
            if (process.server) {
        ...
        async save() {
            ...
            let url = encodeURI(`/api/admin/commissions/${this.$route.params.id}`);
            await axios
                .put(url, {
                    name: this.name,
                    type: this.type,
                ...
                .then((response) => {
                    this.$store.dispatch("toast/makeToast", { type: "success", title: "", message: "کمیسیون با موفقیت ویرایش شد" });
                })
                .catch((e) => {
                    console.log(e);
                    if (typeof e.response !== "undefined" && e.response.data && typeof e.response.data.message === "object") {
