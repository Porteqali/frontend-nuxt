...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <nuxt-link to="/admin/testimonials">نظرات نمایشی</nuxt-link>
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>ویرایش نظر</b></h1>
        </div>
            ...
            <form class="flex flex-col gap-4 flex-grow max-h-full overflow-auto">
                ...
                <div class="flex flex-wrap md:flex-nowrap gap-4 w-full">
                    <div class="flex flex-col gap-2 w-full">
                        ...
                        <input type="text" v-model="fullname" dir="auto" class="p-3 w-full rounded-xl shadow-sm focus:shadow-md bg-coolgray-100" />
                    </div>
                    <div class="flex flex-col gap-2 w-full">
                        <label class="text-sm">
                ...
                <div class="flex flex-col gap-2 w-full">
                    ...
                    <Select :selectedOption.sync="showAs" :options="showAsOptions">
                ...
                <div class="flex flex-col gap-2 w-full">
                    ...
                    <textarea v-model="comment" rows="6" dir="auto" class="p-3 w-full rounded-xl shadow-sm focus:shadow-md bg-coolgray-100"></textarea>
                </div>
            </form>
            <hr class="w-full" />
...
export default {
    layout: "admin",
    head() {
        return { title: "ویرایش نظر - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    data() {
        return {
            ...
            saving: false,
            showAsOptions: {
                teacher: { name: "استاد", value: "teacher" },
                user: { name: "دانشجو", value: "user" },
            },
    ...
    async fetch() {
        ...
        await this.loadTestimonial({ headers }, route);
    },
    computed: {
        userPermissions() {
    ...
    methods: {
        async loadTestimonial(data = {}, route) {
            let url = `/api/admin/testimonials/${route.params.id}`;
            let headers = {};
            if (process.server) {
            ...
            await axios
                ...
                .then((response) => {
                    ...
                    this.showAs = this.showAsOptions[response.data.showAs];
        ...
        async save() {
            ...
            formData.append("showAs", this.showAs.value);
            formData.append("comment", this.comment);

            let url = encodeURI(`/api/admin/testimonials/${this.$route.params.id}`);
            await axios
                .put(url, formData)
                .then((response) => {
                    this.$store.dispatch("toast/makeToast", { type: "success", title: "", message: "نظر با موفقیت ویرایش شد" });
                })
                .catch((e) => {
                    if (typeof e.response !== "undefined" && e.response.data && typeof e.response.data.message === "object") {
                        this.$store.dispatch("toast/makeToast", { type: "error", title: "خطا", message: e.response.data.message[0].errors[0] });
