...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>گروه سوال جدید</b></h1>
        </div>
...
export default {
    layout: "admin",
    head() {
        return { title: "ایجاد گروه سوال جدید - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    methods: {
        async save() {
            ...
            let url = encodeURI(`/api/admin/roadmap-question-category`);
            await axios
                .post(url, { name: this.name, desc: this.desc })
                .then((response) => {
                    this.$store.dispatch("toast/makeToast", { type: "success", title: "", message: "گروه سوال با موفقیت اضافه شد" });
                })
                .catch((e) => {
