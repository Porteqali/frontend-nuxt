...
<template>
    <main class="dashboard_body flex flex-col gap-4 md:p-4 md:py-1">
        <div class="flex flex-wrap md:flex-nowrap items-center gap-2">
            ...
            <img src="/icons/Arrow.svg" width="12" style="transform: rotate(90deg)" />
            <h1 class="text-2xl"><b>ویرایش متادیتا</b></h1>
        </div>
...
export default {
    layout: "admin",
    head() {
        return { title: "ویرایش متادیتا - گروه آموزشی پرتقال" };
    },
    components: {
        Select,
    },
    ...
    async fetch() {
        ...
        await this.loadMetadata({ headers }, route);
    },
    mounted() {
    ...
    methods: {
        async loadMetadata(data = {}, route) {
            let url = `/api/admin/metadata/${route.params.id}`;
            let headers = {};
            if (process.server) {
        ...
        async save() {
            ...
            let url = encodeURI(`/api/admin/metadata/${this.$route.params.id}`);
            await axios
                .put(url, {
                    page: this.page,
                    title: this.title,
                ...
                .then((response) => {
                    this.$store.dispatch("toast/makeToast", { type: "success", title: "", message: "متادیتا با موفقیت ویرایش شد" });
                })
                .catch((e) => {
                    if (typeof e.response !== "undefined" && e.response.data && typeof e.response.data.message === "object") {
                        this.$store.dispatch("toast/makeToast", { type: "error", title: "خطا", message: e.response.data.message[0].errors[0] });
